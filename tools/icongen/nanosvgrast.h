...
static void dumpEdges(NSVGrasterizer* r, const char* name)
{
 ...
 for (i = 0; i < r->nedges; i++) {
 e = &r->edges[i];
 fprintf(fp ," ", e->x0,e->y0, e->x1,e->y1);
 }

 for (i = 0; i < r->npoints; i++) {
 if (i+1 < r->npoints)
 fprintf(fp ," ", r->points[i].x, r->points[i].y, r->points[i+1].x, r->points[i+1].y);
 fprintf(fp ," ", r->points[i].x, r->points[i].y, r->points[i].flags == 0 ? "#f00" : "#0f0");
 }
